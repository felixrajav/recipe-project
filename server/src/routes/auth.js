...
export const verifyToken = (req, res, next) => {
  ...
  if (authHeader) {
    jwt.verify(authHeader, "secret", async (err) => {
      if (err) {
      ...
      if (userId !== req.userId) { // Check if userId matches the authenticated user
        return res.status(403).json({ message: "You do not have permission to edit this recipe" });
      }

      next(); // Proceed with the request if the userId is valid
    });
  } else {
